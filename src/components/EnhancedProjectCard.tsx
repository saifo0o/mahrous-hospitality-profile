...
const EnhancedProjectCard: React.FC<EnhancedProjectCardProps> = ({
  ...
  return (
    <motion.div
      ...
      className="group relative bg-card rounded-sm overflow-hidden border border-border/50 hover:border-accent/50 transition-colors duration-500"
      whileHover={{ y: -6 }}
    >
      {/* Image Container */}
      <div className="relative h-72 overflow-hidden">
        ...
        {/* Category Badge */}
        <motion.div
          className={`absolute top-4 ${isRTL ? 'left-4' : 'right-4'} bg-accent text-accent-foreground text-xs font-bold px-4 py-2 rounded-sm border border-accent`}
        ...
        <motion.button
          ...
          className="absolute inset-0 m-auto w-16 h-16 bg-card/95 hover:bg-card rounded-full shadow-md opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center border border-border"
      ...
      <div className="p-6 space-y-4 bg-gradient-to-b from-card to-muted/30">
        <h3 className="font-bold text-xl text-foreground group-hover:text-accent transition-colors duration-300 line-clamp-2">
          {title}
        </h3>
