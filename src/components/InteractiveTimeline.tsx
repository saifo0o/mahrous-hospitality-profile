...
const InteractiveTimeline: React.FC<InteractiveTimelineProps> = ({ events, className = '' }) => {
  ...
  return (
    <div className={`relative ${className}`}>
      {/* Timeline line */}
      <div className={`absolute top-0 bottom-0 w-px bg-luxury-gold/30 ${isRTL ? 'right-6' : 'left-6'}`} />
      ...
      <div className="space-y-8">
        {events.map((event, index) => (
          ...
          >
            {/* Timeline node */}
            <motion.div
              className="relative z-10 flex items-center justify-center w-12 h-12 rounded-full bg-card border-2 border-luxury-gold"
              animate={{
                backgroundColor: hoveredEvent === event.id ? 'hsl(var(--accent))' : undefined,
                color: hoveredEvent === event.id ? 'hsl(var(--accent-foreground))' : undefined
              }}
            ...
            {/* Content card */}
            <motion.div
              className="flex-1 bg-card rounded-sm p-6 border border-border"
              whileHover={{ y: -4 }}
              animate={{
                borderColor: hoveredEvent === event.id ? 'hsl(var(--accent))' : undefined
              }}
              transition={{ duration: 0.2 }}
            >
              <div className="flex items-start justify-between mb-3">
                ...
                <span className="text-lg font-bold text-luxury-gold bg-luxury-gold/10 px-3 py-1 rounded-sm">
                  {event.year}
                </span>
              </div>
